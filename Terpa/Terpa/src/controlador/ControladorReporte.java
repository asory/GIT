...
public class ControladorReporte implements ActionListener {
 ...
 private TableRowSorter trsfiltro;
	...
	public void filtro() {
		...
		case 1:
	trsfiltro.setRowFilter(RowFilter.regexFilter(vreport.getTextFiltrar().getText(), 1));
		case 2: 
			trsfiltro.setRowFilter(RowFilter.regexFilter(vreport.getTextFiltrar().getText(), 2));
		case 3:
			trsfiltro.setRowFilter(RowFilter.regexFilter(vreport.getTextFiltrar().getText(),3 ));
		case 4:
			trsfiltro.setRowFilter(RowFilter.regexFilter(vreport.getTextFiltrar().getText(), 4));
		case 5:
			trsfiltro.setRowFilter(RowFilter.regexFilter(vreport.getTextFiltrar().getText(), 5));
		case 6:
			trsfiltro.setRowFilter(RowFilter.regexFilter(vreport.getTextFiltrar().getText(), 6));
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		vreport.getTextFiltrar().addKeyListener(new KeyAdapter() {
			public void keyReleased(final KeyEvent e) {
				String cadena = (vreport.getTextFiltrar().getText()).toUpperCase();
				vreport.getTextFiltrar().setText(cadena);
				vreport.getTextFiltrar().repaint();
				filtro();
				}
				});
				trsfiltro = new TableRowSorter(vreport.getTable().getModel());
