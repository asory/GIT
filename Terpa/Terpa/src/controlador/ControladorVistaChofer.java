...
public class ControladorVistaChofer implements ActionListener {
	...
	public static ControladorVistaChofer getInstancia() {
		if (instancia == null) {
			instancia = new ControladorVistaChofer();
		}
	...
	private void agregarChofer() {
		...
		Chofer cho = new Chofer(vcho.getNombreC(), vcho.getApellidoC(),
				vcho.getTelefonoC(), vcho.getCiC(), vcho.getIdC(), false,
				vcho.getSocioC());

		if (vcho.getRifCoop().isEmpty() || vcho.getCiC().isEmpty()
				|| vcho.getApellidoC().isEmpty()
				|| vcho.getTelefonoC().isEmpty() || vcho.getSocioC().isEmpty()
				|| vcho.getIdC().isEmpty()) {
